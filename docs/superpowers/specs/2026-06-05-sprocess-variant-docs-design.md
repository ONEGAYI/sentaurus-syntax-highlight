...
| 类别 | 数量 | 处理方式 |
|------|------|----------|
| **复数变体 Alias** | 8 个 | 文档 JSON 内联 alias 条目，HoverProvider 预处理 |
...
| Kind | 关键词 | aliasOf |
|------|--------|---------|
| KEYWORD2 | `ambients=` | `ambient` |
| KEYWORD3 | `contacts` | `contact` |
| KEYWORD3 | `interfaces` | `interface` |
| KEYWORD3 | `masks` | `mask` |
| KEYWORD2 | `masks=` | `mask` |
| KEYWORD2 | `points=` | `point` |
| KEYWORD2 | `polygons=` | `polygon` |
| KEYWORD2 | `regions=` | `region` |
...
| 文件 | 变更内容 |
|------|----------|
| `syntaxes/sprocess_command_docs.json` | 新增 8 个 alias 条目 + 51 个点号变体文档 |
...
6. **Alias 条目同步写入**：8 个 alias 条目写入两个 JSON 文件
