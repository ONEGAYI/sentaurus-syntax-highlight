...
为 Synopsys Sentaurus TCAD 工具链提供语法高亮、自动补全和语义功能的 VSCode 扩展。支持 SDE、SDEVICE、SDEVICE PAR、SPROCESS、EMW、INSPECT、SVISUAL 七种工具的命令文件。
...
| 注释识别 | `#` 通用注释；SDE 额外支持 `;`（Scheme 注释）；SDevice/EMW/SProcess/Inspect/SDevicePAR 额外支持 `*` |
| SWB 变量 | 高亮 `@Var@`、`@param:+2@` 等 Sentaurus Workbench 参数替换语法 |
| 括号匹配 | `{}` `[]` `()` 自动配对 |
| **帮助文档** | **内置文档阅读器**，三栏布局（导航树 + 文章 + 大纲），支持搜索高亮和状态持久化 |

### 内置帮助文档

通过命令面板执行 `Sentaurus: Open Help` 打开内置帮助文档阅读器。基于 Webview 的三栏布局，提供流畅的文档浏览体验：

- **左侧导航树**：目录结构一键跳转，支持折叠/展开
- **中间文章区**：Markdown 渲染，链接自动拦截跳转
- **右侧大纲面板**：标题层级一览，IntersectionObserver 追踪当前阅读位置
- **全文搜索高亮**：输入关键词即时高亮匹配，支持 ▲▼/Enter/Esc 导航
- **状态持久化**：关闭面板后重新打开自动恢复上次阅读位置、搜索词和侧栏状态

![帮助文档阅读器](assets/pics/vscode%20插件效果演示%20-%20内置帮助文档.gif)
...
2. 选择对应语言（如 `sde`、`sdevice`、`sdevicepar`、`sprocess`、`emw`、`inspect`）
